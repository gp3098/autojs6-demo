...
export class DouyinScheduler {
  ...
  private reducer(state: DouyinState, action: DouyinAction): DouyinState {
    switch (action.type) {
      ...
      case "SET_AWAITING_RETURN":
        return {
          ...state,
          awaitingTaskReturn: action.value,
          timestamp: Date.now(),
        };
      case "SET_BUSY":
  ...
  private detectAndDispatchPage() {
    ...
    if (hasLotteryMask) {
      ...
      hasSignInMask &&
      (page === "TASK_HOME" || page === "TASK_PANEL" || page === "TASK_LIST")
    ) {
    ...
    } else if (hasLuckyStrikeExclusiveBoundsMask) {
  ...
  private handleGlobalPopups() {
    ...
    } else if (state.overlay === "LUCKY_STRIKE_EXCLUSIVE_BONUS_MASK") {
  ...
  private handleLuckyBoundsMask(): boolean {
    if (String(currentPackage() || "") !== this.appPackage) {
      return false;
    }
  ...
  private handleTaskPanel() {
    if (this.handleCollectGoldAction()) {
      console.log("handleTaskPanel, handleCollectGoldAction");
      return;
    }
    this.openTaskList();
  }
  ...
  private handleAdConfirmDialog(): boolean {
    ...
    const continueUi =
      ...
      textContains("再看").findOnce() ||
      textContains("继续领奖励").findOnce();
    if (continueUi) {
  ...
  private pickVisibleTask(finishedTaskRuns: {
